  ...
  const handleSave = async () => {
    try {
      const payload = students.map(student => ({
        ...
        amount: student.amount || 0,  // Ensure feeAmount is set to 0 if empty
      }));
  
      console.log("Payload being sent:", payload); // Log the payload to verify its contents
  
      const response = await fetch('http://127.0.0.1:59033/fees/saveFeeHeads', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer YOUR_ACCESS_TOKEN',  // Use actual token if necessary
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ feeHeads: payload }),  // Make sure the structure matches backend expectations
      });
  
      if (!response.ok) {
        const errorData = await response.json();  // Read the response as JSON if available
