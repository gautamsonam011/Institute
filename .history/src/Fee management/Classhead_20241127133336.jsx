...
const StudentTable = () => {
  ...
  const getAuthToken = async () => {
    ...
    const token = localStorage.getItem('authToken'); // Or use any method you have to retrieve the token
    return token;
  };
  ...
  useEffect(() => {
    const fetchFeeHeadData = async () => {
      try {
        ...
        const response = await fetch('http://127.0.0.1:59033/fees/addFeeHeadAll', {
          method: 'GET',
          headers: {
