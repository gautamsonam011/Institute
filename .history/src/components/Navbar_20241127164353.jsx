...
const Navbar = () => {
  ...
  const handleDownloadCertificate = async (e) => {
    ...
    try {
      ...
      // Check the content type to make sure it's a PDF
      if (response.headers.get("content-type") !== "application/pdf") {
        throw new Error("Failed to download PDF: Invalid file type.");
      }

      // Trigger the download of the PDF file
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `certificate_${studentNo}.pdf`;  // Assuming the certificate is a PDF file
      link.click();
    } catch (err) {
